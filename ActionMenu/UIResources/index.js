...
var wait_joystick_recenter = false; // prevent users to select items by mistake for certain widgets VR
var wait_joystick_notcenter = false; // prevent users to select items by mistake for certain widgets Desktop
var was_in_deadzone = false;
var update_to_items = {}; // items registry to sync game<>menu state

// wait for VR user to recenter joystick before accepting input
// center joystick for Desktop user, but wait for them to leave deadzone before accepting input
function center_joystick() {
	if (settings.in_vr) {
		wait_joystick_recenter = true;
		return;
	}
	wait_joystick_notcenter = true;
